...
for(const t of['centre, angle, and direction','same perpendicular distance','moves the same distance and direction','Every point follows the same translation'])check.includes(t)?pass(`Check Yourself transformation reasoning ${t}`):fail(`Check Yourself missing ${t}`);

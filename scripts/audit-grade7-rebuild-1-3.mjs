...
if(fs.existsSync(route13)){const html=fs.readFileSync(route13,'utf8');sections.every(id=>html.includes(`id="${id}"`))?pass('Lesson 1.3 built route contains all ten anchors'):fail('Lesson 1.3 built route missing anchors');html.includes('Transformations')?pass('Lesson 1.3 built route title present'):fail('Lesson 1.3 title missing');const m=html.match(/<script type="application\/json" data-bank[^>]*>(.*?)<\/script>/s);if(m){try{const bank=JSON.parse(m[1].replace(/&quot;/g,'"').replace(/&amp;/g,'&'));bank.length>=36?pass(`Check Yourself bank size = ${bank.length}`):fail(`Check Yourself bank too small = ${bank.length}`)}catch(e){fail(`could not parse Check Yourself bank: ${e.message}`)}}else fail('Check Yourself bank script missing')}else fail('Lesson 1.3 built route missing');
